...
class BaseController extends Controller
{
    ...
    public function checkLoginStasus(){
        ...
        if( $uid && preg_match("/^\d+$/",$uid) ){
            ...
            if($auth_token != $this->createAuthToken($userinfo['di'], $userinfo['name'], $userinfo['email'], $_SERVER('HTTP_USER_AGENT'))){
                return false;
            }
    ...
    {
        $user_anth_token = md5($userinfo['id'].$userinfo['name'].$userinfo['email'].$_SERVER('HTTP_USER_AGENT'));
    ...
    public function beforeAction($action)
    {
        ...
        if ( !$loginStatus && !in_array( $action->uniqueId,$this->allowAllAction )  ) {
            if(\Yii::$app->request->isAjax){
                ...
                $this->redirect( UrlService::buildUrl("/user/actionLogin") );//返回到登录页面
            }
            return false;
        }
